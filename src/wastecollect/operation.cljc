(ns wastecollect.operation
  "RouteOperationActor -- one waste-collection coordination request =
  one supervised actor run, expressed as a langgraph-clj StateGraph.
  The advisor (Route Advisor) is sealed into a single node (:advise);
  its proposal is ALWAYS routed through the Waste Operations Governor
  ...
  each operation is auditable and checkpointed. A hauler's lifecycle is
  advanced by MANY ops (route-pickup logging / sorting-result filing /
  dispatch scheduling / contamination escalation), each independent.

  Human-in-the-loop = real approval workflow:
  `interrupt-before #{:request-approval}` pauses the actor and hands the
  decision to a human route coordinator. The approver resumes with
...
(defn build
  "Compiles a RouteOperationActor graph bound to `store`.
  opts:
    :advisor      -- a `wastecollect.advisor/Advisor` (default: mock-advisor)
  ...
  (-> (g/state-graph
      ...
      ;; Route Advisor inference (the contained intelligence node) -- proposal only.
      (g/add-node :advise
        (fn [{:keys [request]}]
          (let [p (advisor/-advise advisor store request)]
            {:proposal p :audit [(advisor/trace request p)]})))

      ;; Waste Operations Governor -- independent censor.
      (g/add-node :govern
